...
class ImageScanner:
    ...
    def save_crop_points_json(self):
        out_path = self.output_dir / 'crop_points.json'
        with out_path.open('w', encoding='utf-8') as f:
            json.dump(self.crop_records, f, ensure_ascii=False, indent=2)
        print(f'已保存裁剪点信息: {out_path.name}')
    ...
    def update_display(self):
        ...
        self.display_image = cv2.resize(
            crop, (render_w, render_h), interpolation=cv2.INTER_LINEAR
        )
            ...
            while not image_completed and self.running:
                    ...
                    if len(self.points) == 4:
                        ...
                        try:
                            ...
                            coordinates = {
                                ...
                                '2': [
                                    float(self.points[1][0]),
                                    float(self.points[1][1]),
                                ],
                                '3': [
                                    float(self.points[2][0]),
                                    float(self.points[2][1]),
                                ],
                                '4': [
                                    float(self.points[3][0]),
                                    float(self.points[3][1]),
                                ],
                            }
                ...
                if key != 255:
                    if key == ord('q'):  # Quit
                        ...
                        self.process_command('zoom +')
                    elif key == ord('-'):
                        self.process_command('zoom -')
                    elif key == ord('w'):
                        self.process_command('up')
                    elif key == ord('s'):
                        self.process_command('down')
                    elif key == ord('a'):
                        self.process_command('left')
                    elif key == ord('d'):
                        self.process_command('right')
                    elif key == ord('z'):
